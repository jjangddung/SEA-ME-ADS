...
private:
  ...
  void cloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
  {
    ...
    cv::Mat img = latest_image_.clone();
    cv::Mat mask = white_mask_;
    int h = img.rows;
    int w = img.cols;
    ...
    for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z, ++i)
    {
      ...
      if (u_i >= 0 && u_i < w && v_i >= 0 && v_i < h)
      {
        ...
        {
          // 이미지에 초록 점 그리기
        //   cv::circle(img, cv::Point(u_i, v_i), 2, cv::Scalar(0, 255, 0), -1);
    ...
    try
    {
      cv::imshow("lane points on image (cpp)", img);
      cv::waitKey(1);
    }
    catch (...)
    {
